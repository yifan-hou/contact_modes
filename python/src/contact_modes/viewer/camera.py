...
class TrackballCamera(object):
    ...
    def _view(self,cam_radius=None,cam_focus=None,cam_up=None):
        ...
        if cam_up:
            self.cam_up = cam_up
        view = glm.lookAt(glm.vec3(self.cam_eye[0],self.cam_eye[1],self.cam_eye[2]),
                          glm.vec3(self.cam_focus[0],self.cam_focus[1],self.cam_focus[2]),
                          glm.vec3(self.cam_up[0],self.cam_up[1],self.cam_up[2]))
        # rotate this view by the current orientation
        m = glm.mat4(*self._matrix())
        return view * m
