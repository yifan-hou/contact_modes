...
\begin{algorithm}[t]
    ...
    \begin{algorithmic}[1]
        ...
        \State $M \gets \Call{Conv-Hull}{\mathcal{V}(A)}$
...
and for our system, it amounts to solving the linear program

\begin{alignat}{2}
    &\!\!\min_{x} && {\begin{bmatrix}0& \cdots& 0& 1\end{bmatrix} \cdot x} \label{eq:itm-obj}\\
    &\textup{s.t.} & & 
      \begin{bmatrix}
        f_g \\ \alpha \\ \beta \\ \gamma
      \end{bmatrix} = 
      \begin{bmatrix}
        A & 0 & N_o^T & T_o^T & 0 \\
        N_o & -N_m & 0 & 0 & 0 \\
      ...
      \end{bmatrix}
      \begin{bmatrix}
        v\\ \dot{q}_m \\ \lambda_n \\ \lambda_f \\ \sigma
      \end{bmatrix}\label{eq:itm-eq}\\
      & & 
        0\leq &
          \begin{bmatrix}
            \alpha \\ \beta \\ \gamma
